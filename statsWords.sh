...
### over the words. Then we check if the word ends in a period or a comma and if so, the character is 
### removed. After that we check if its the first word in the text and in the affirmative case, we 
### save the lenght of this first word to compare with the rest later. If it's not the first iteration
### we compare the lenght of this new word with the length stored in the variables created before.
...
### to make the average later.
### Finally we get the word count with the wc -w command and calculate the average. Then the results
### are printed.
